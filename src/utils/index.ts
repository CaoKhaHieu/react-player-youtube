...
export const getConfigSetting = (
  key: string,
  otherValue: { label: string; value: string | number },
) => {
  const dataLocal = getDataLocal();
  return {
    label:
      dataLocal && dataLocal[key] ? dataLocal[key]?.label : otherValue.label,
    value:
      dataLocal && dataLocal[key] ? dataLocal[key]?.value : otherValue.value,
  };
};
